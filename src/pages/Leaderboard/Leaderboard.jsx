...
const leaderboardData = [
  { id: 1, name: 'Rohan V.', xp: 15240, streak: 42, focus: 128, score: 92, dept: 'CS', rising: true },
  { id: 2, name: 'Priya S.', xp: 14900, streak: 28, focus: 110, score: 88, dept: 'AI', rising: false },
  { id: 3, name: 'Ananya J.', xp: 13200, streak: 15, focus: 95, score: 85, dept: 'CS', rising: true },
  { id: 4, name: 'Sneha K.', xp: 12800, streak: 21, focus: 88, score: 82, dept: 'ECE', rising: false },
  { id: 5, name: 'Karan M.', xp: 11500, streak: 12, focus: 76, score: 78, dept: 'MECH', rising: true },
  { id: 6, name: 'Meera D.', xp: 10200, streak: 8, focus: 64, score: 75, dept: 'CIVIL', rising: false },
];

export default function Leaderboard() {
  const [filter, setFilter] = useState('All');
  const currentUserId = 4; // Mocking Sneha K. as the current user
      ...
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'center', gap: '32px', marginBottom: '80px', padding: '0 40px' }}>
        ...
        <motion.div {...s(2)} style={{ width: '220px', textAlign: 'center' }}>
          ...
          <GlassCard elevated style={{ padding: '24px', background: 'linear-gradient(to top, rgba(148,163,184,0.1), transparent)', border: '1px solid rgba(148,163,184,0.2)' }}>
            <div style={{ fontSize: '18px', fontWeight: 900, marginBottom: '4px' }}>Priya S.</div>
            <div style={{ fontSize: '13px', color: 'var(--accent-primary)', fontWeight: 800 }}>14.9k XP</div>
        ...
        <motion.div {...s(1)} style={{ width: '280px', textAlign: 'center', zIndex: 10 }}>
          ...
          <GlassCard elevated style={{ padding: '32px', background: 'linear-gradient(to top, rgba(245,158,11,0.1), transparent)', border: '1px solid rgba(245,158,11,0.3)' }}>
            <div style={{ fontSize: '22px', fontWeight: 950, marginBottom: '6px' }}>Rohan V.</div>
            <div style={{ fontSize: '15px', color: 'var(--accent-primary)', fontWeight: 800 }}>15.2k XP</div>
        ...
        <motion.div {...s(3)} style={{ width: '220px', textAlign: 'center' }}>
          ...
          <GlassCard elevated style={{ padding: '24px', background: 'linear-gradient(to top, rgba(146,64,14,0.1), transparent)', border: '1px solid rgba(146,64,14,0.2)' }}>
            <div style={{ fontSize: '18px', fontWeight: 900, marginBottom: '4px' }}>Ananya J.</div>
            <div style={{ fontSize: '13px', color: 'var(--accent-primary)', fontWeight: 800 }}>13.2k XP</div>
      ...
      <motion.div {...s(4)} style={{ marginBottom: '40px' }}>
        <GlassCard elevated style={{ padding: '24px 32px', background: 'rgba(99,102,241,0.08)', border: '1px solid rgba(99,102,241,0.3)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
             ...
             <div>
                <div style={{ fontSize: '18px', fontWeight: 900 }}>Synchronized as Rank #4</div>
                <div style={{ fontSize: '14px', color: 'var(--text-secondary)', fontWeight: 600 }}>Accelerating at <span style={{ color: 'var(--accent-success)' }}>+142 XP / session</span></div>
             </div>
          </div>
          <div style={{ textAlign: 'right' }}>
             <div style={{ fontSize: '16px', fontWeight: 900, color: 'var(--accent-danger)', letterSpacing: '0.02em' }}>340 XP BEHIND PRIYA 🔥</div>
             <div style={{ fontSize: '12px', color: 'var(--text-dim)', marginTop: '4px', fontWeight: 700 }}>ADHYETA MILESTONE: 13,000 XP</div>
          </div>
        </GlassCard>
      </motion.div>
              ...
              <tbody>
                {leaderboardData.map((row) => (
                  <tr key={row.id} style={{ 
                    ...
                    <td style={{ padding: '24px 32px', fontWeight: 950, color: row.id <= 3 ? 'var(--accent-warn)' : 'var(--text-dim)', fontSize: '16px', fontFamily: 'var(--font-mono)' }}>#{row.id}</td>
