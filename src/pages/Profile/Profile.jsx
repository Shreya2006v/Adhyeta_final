      ...
      <GlassCard elevated style={{ 
        ...
        <div style={{ flex: 1 }}>
          ...
          <p style={{ color: 'var(--text-secondary)', fontSize: '16px', fontWeight: 600 }}>
            {studentProfile.year} · {studentProfile.branch} Domain
          </p>
          <div style={{ display: 'flex', gap: '16px', marginTop: '16px' }}>
            <Badge color="primary">RANK #{studentProfile.rank}</Badge>
            <Badge color="success">{studentProfile.goal}</Badge>
          </div>
        </div>

        <div style={{ display: 'flex', gap: '32px' }}>
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: '28px', fontWeight: 800, color: 'var(--accent-primary)' }}>{studentProfile.xp}</div>
            <div style={{ fontSize: '10px', color: 'var(--text-dim)', fontWeight: 800 }}>XP</div>
          </div>
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: '28px', fontWeight: 800, color: 'var(--accent-warn)' }}>{studentProfile.streak}</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
        <GlassCard elevated style={{ padding: '24px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
             {[
               { label: 'Avg Accuracy', val: studentProfile.avgAccuracy + '%', color: 'var(--accent-success)' },
               { label: 'Tests Done', val: studentProfile.testsCompleted, color: 'var(--accent-primary)' },
               { label: 'Study Hours', val: studentProfile.totalStudyHours + 'h', color: 'var(--accent-secondary)' },
             ].map(s => (
        ...
        <GlassCard elevated style={{ padding: '24px' }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px' }}>
            {studentProfile.strongSubjects.map(s => (
            ...
            {studentProfile.weakSubjects.map(s => (
