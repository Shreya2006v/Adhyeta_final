...
export default function Register() {
  ...
  const [formData, setFormData] = useState({
    student: {
      ...
      collegeInfo: { type: '', branch: '', year: '', name: '', university: '' },
  ...
  return <StudentFlow step={step} setStep={setStep} data={formData.student} update={updateStudent} onBack={() => setMode('select')} onLaunch={() => navigate('/dashboard')} />;
}

function StudentFlow({ step, setStep, data, update, onBack, onLaunch }) {
        ...
        <div style={{ maxWidth: '640px', width: '100%' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '64px', alignItems: 'center' }}>
            ...
            {step < 5 ? (
              ...
              <Button variant="glow" size="lg" icon={<LogIn size={20} />} onClick={onLaunch}>Initialize Path</Button>
            )}
          </div>
        </div>
      </div>
        ...
        {cat === 'college' && (
          <>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              ...
              <input
                className="premium-input" placeholder="Enter your college/university name..."
                style={{ height: '52px' }}
                value={data.collegeInfo.name} onChange={e => updateInfo('name', e.target.value)}
              />
            </div>
          </>
        )}
...
function InstitutionFlow({ data, update, onBack, onLaunch }) {
  const [isDragging, setIsDragging] = useState(false);

  const handleFile = (e) => {
    // Basic file handling logic here
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg-void)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px' }}>
      <div style={{ maxWidth: '840px', width: '100%' }}>
        <GlassCard elevated style={{ padding: '56px', border: '1px solid rgba(99,102,241,0.15)' }}>
          ...
          <Button variant="glow" size="lg" style={{ width: '100%', height: '60px', fontSize: '18px', borderRadius: '18px' }} onClick={onLaunch}>Initialize Institution Node</Button>
        </GlassCard>
      </div>
    </div>
  );
