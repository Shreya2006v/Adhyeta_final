...
export default function Login() {
  ...
  const loginMutation = useMutation({
    ...
    onSuccess: async (data) => {
      ...
      try {
        ...
        const userObj = meRes.data.data || meRes.data; // unwrap { success, message, data: {...} }
        setAuth(userObj, data.access, data.refresh);
        if (userObj.role === 'student' || role === 'student') navigate('/dashboard');
        else if (role === 'teacher') navigate('/teacher');
        else if (role === 'hod') navigate('/hod');
        else navigate('/admin');
      } catch (err) {
        setAuth({ email, role }, data.access, data.refresh);
        navigate('/dashboard');
      }
    },
    onError: (err) => {
  ...
  const handleSignIn = () => {
    ...
    setError('');
    loginMutation.mutate({ username: email, email, password });
  };
          ...
          <GlassCard elevated style={{ padding: window.innerWidth < 480 ? '24px' : '48px', borderRadius: '32px', border: '1px solid rgba(255,255,255,0.05)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
              ...
              <Button 
                variant="glow" 
                size="lg" 
                style={{ height: '56px', fontSize: '16px', opacity: loginMutation.isPending ? 0.7 : 1 }} 
                onClick={handleSignIn} 
                icon={loginMutation.isPending ? undefined : <LogIn size={20} />}
                disabled={loginMutation.isPending}
              >
                {loginMutation.isPending ? 'Authenticating...' : 'Login to ADHYETA'}
              </Button>
